function [bool, K, diagnostics, gamma, info] = isInformH2(X, U, Phi, C, D, tolerance, options)
%ISINFORMH2 checks if the daat is informative for H2 control.
...
%                       16: Matrix inequality 3 does not hold
%                       32: trace(Z) !<= gamma
%                       64: a !>= 0
%                      128: b !>  0
%                      256: Y !>  0
...
%          EmptyStateData         : Provide a non empty state measurement matrix.
    ...
    if min(eig(value(cond_matrix_3))) < 0
        % The matrix constraint 3 was not positive semi definite
        info = info + 16;
    end
    if value(trace(Z)) >= value(gamma) + tolerance
        % trace(Z) !<= gamma
        info = info + 32;
    end
    if value(a) < 0
